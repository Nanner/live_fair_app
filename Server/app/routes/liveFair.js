...
module.exports = function(server){
    ...
    server.route({
        ...
        config:{
            ...
            handler: function (request, reply) {
                if(!request.payload.interests || !request.params.LiveFairID || !request.params.UserID){
                    ...
                    if(userType === "visitor") {
                        ...
                        VisitorLiveFair.create({
                            ...
                            for(var i = 0; i<interests.length; i++){
                                interestPromises.push(LiveFairVisitorInterest.create({
                                    'liveFairIDref':LiveFairID,
                                    'interestIDref':interests[i],
                                    'visitorIDref':UserID
                                }));
                            }
                            Promise.all(interestPromises).then(function() {
                        ...
                        Stands.create({
                            ...
                            var interestPromises = [];
                            for(var i = 0; i<interests.length; i++){
                                interestPromises.push(LiveFairCompanyInterest.create({
                                    'liveFairIDref':LiveFairID,
                                    'interestIDref':interests[i],
                                    'companyIDref':UserID
                                }));
